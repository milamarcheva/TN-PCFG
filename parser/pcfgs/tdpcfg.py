...
class Fastest_TDPCFG(PCFG_base):
    ...
    def _inside(self, rules, lens, mbr=False, viterbi=False, marginal=False, s_span=None):
        ...
        def merge(Y, Z, y, z, indicator):
            ...
            # Allow span indicators to carry one slot per nonterminal when
            # collecting label marginals; otherwise treat them as scalars.
            if indicator.dim() == 2:
                indicator = indicator.unsqueeze(-1)
            if indicator.size(-1) == 1:
                indicator = indicator.expand_as(Y[..., 0])
        ...
        if s_span is None:
            ...
            if mbr or viterbi or label_marginal:
