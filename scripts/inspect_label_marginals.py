...
def _normalize_distribution(dist: torch.Tensor, *, allow_raw: bool) -> torch.Tensor:
    """Return a probability distribution for ``dist`` or the raw values when requested."""
...
def _topk_distribution(
    dist: torch.Tensor, k: int, labels: Sequence[str], *, allow_raw: bool
) -> Tuple[List[Tuple[str, float]], float]:
    if dist.numel() == 0:
        return [], 0.0

    probs = _normalize_distribution(dist, allow_raw=allow_raw)
    k = min(k, probs.numel())
    if k == 0:
        return [], 0.0
...
def _summarise_spans(
    ...
    *,
    ...
    def handle_span(start: int, end: int) -> None:
        ...
        top, best_prob = _topk_distribution(dist, topk, labels, allow_raw=allow_raw)
        nonlocal max_seen
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set, report the raw autograd marginals without re-normalizing them in this script. "
